...
function BuildSimpleGUI() {
   ...
   guiCode += arrFiles[0];
   guiCode += '" size="30" id="urlToLoad"/><br/>'
   ...
   guiCode += '</select>'
      ...
      +'</select>' 
      +'</form>';
   guiCode += '<form method="get" action="';
   guiCode += arrFiles[0];
   guiCode += '">'
      +'<button style="padding:2px; margin-left:10px; margin-top:5px;" type="submit">Download ROOT file</button>'
      +'</form>';
    guiCode += '<br/>'
      +'<div id="browser"></div>'
      +'</div>'
